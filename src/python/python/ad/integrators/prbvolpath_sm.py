...
class _SegmentRecord:
    ...
    DRJIT_STRUCT = {'origin': mi.Point3f, 'direction': mi.Vector3f,
                    'interval': mi.Float, 'v': mi.Float, 'nee': mi.Point2f,
...
class PRBVolpathSMIntegrator(RBIntegrator):
    ...
    def sample(self,
        ...
        while dr.hint(active,
            ...
            with dr.resume_grad(when=not is_primal):
                ...
                if dr.hint(not is_primal, mode='scalar'):
                    ...
                    # more than one segment; it keeps one with probability v / wsum and
                    # weights it by wsum / v.
                    ...
                    rec.interval, rec.v, rec.nee = interval, v, nee_dir_sample
                    ...
                    dr.scatter(records, rec, slot, keep)
        ...
        if dr.hint(defer, mode='scalar'):
            self._flush_deferred_probes(scene, records, dfr_lane_wsum, n_append,
    ...
    def _flush_deferred_probes(self, scene, records, dfr_lane_wsum, n_append, seed):
        ...
        adj_trans = rec.adj_trans * comp
        adj_scatt = rec.adj_scatt * comp
